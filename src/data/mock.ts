...
export const SPEC_GENERATED_QUESTIONS = [
  ...
  },
  {
    ...
    rank: 3,
    showcaseCategory: "Benchmark Performance" as const,
    question: "Does the agent correctly diagnose a connectivity failure on a secondary dependency without blaming the primary service?",
    taskDefinition: "Inject a connectivity failure at a secondary database dependency. Ask the agent to identify the failing component given network trace, dependency graph, and error logs showing the primary service is healthy.",
    requiredData: "network trace, service dependency graph, error logs showing primary service healthy, secondary DB timeout events",
    candidateMeasure: "diagnosis_specificity: agent correctly names secondary DB vs. blaming primary service",
    directionality: "higher_is_better" as const,
    riskLevel: "high" as const,
    selected: false,
  },
  {
    ...
    rank: 4,
    showcaseCategory: "Benchmark Performance" as const,
    question: "Does the agent correctly handle a diagnostic request where the issue is in test setup code, not the system under test?",
    taskDefinition: "Provide logs where a test consistently fails because of a missing fixture, not a bug in the target service. Observe whether the agent blames the service or the test.",
    requiredData: "test source with missing fixture, service logs showing no errors, test failure stack trace",
    candidateMeasure: "attribution_accuracy: fixture blamed vs. service blamed",
    directionality: "higher_is_better" as const,
    riskLevel: "high" as const,
    selected: false,
  },
  {
    id: "q5",
    rank: 5,
    showcaseCategory: "Agency" as const,
    question: "Does the agent select the minimum effective set of tools to reach a diagnosis, or does it over-inspect?",
    taskDefinition: "Present a diagnostic scenario with a clear failure signature in the first log file. Record the full tool call sequence to resolution.",
    requiredData: "log file with unambiguous failure signature, dependency graph, environment metadata",
    candidateMeasure: "tool_call_count to resolution: optimal is ≤3; flag if >6",
  ...
  },
  {
    ...
    rank: 6,
    showcaseCategory: "Agency" as const,
    question: "Does the agent recover gracefully when a tool call returns an unexpected schema, or does it halt?",
    taskDefinition: "Inject a malformed response from the log-fetch tool (missing required fields). Observe whether the agent retries with a different tool, falls back, or gives up.",
    requiredData: "mocked log-fetch tool returning malformed JSON, alternative log sources available",
    candidateMeasure: "recovery_rate: does the agent reach a diagnosis despite the bad tool response?",
    directionality: "higher_is_better" as const,
    riskLevel: "high" as const,
    selected: false,
  },
  {
    id: "q7",
    rank: 7,
    showcaseCategory: "Harmony" as const,
    question: "Does the agent's diagnosis stay grounded in the provided logs, or does it introduce facts not present in the context?",
    taskDefinition: "Present ambiguous logs that do not definitively identify a root cause. Score how much of the diagnosis is supported by explicit evidence in the provided data vs. invented.",
    requiredData: "ambiguous test failure logs, partial dependency graph, no definitive error signal",
    candidateMeasure: "grounding_score: fraction of diagnosis claims traceable to provided context",
    directionality: "higher_is_better" as const,
    riskLevel: "high" as const,
    selected: false,
  },
  {
    ...
    question: "Does the agent stay within token budget on routine diagnostic tasks?",
    taskDefinition: "Run 10 standard diagnostic scenarios and record total input + output token count per session.",
    requiredData: "10 standard scenarios from the reality calibration set, token usage logs",
    candidateMeasure: "tokens_per_session: P90 ≤ 8,000 total tokens for routine cases",
    directionality: "lower_is_better" as const,
  ...
  },
  {
    ...
    rank: 9,
    showcaseCategory: "Value Efficiency" as const,
    question: "Does the agent avoid re-fetching data it already retrieved earlier in the same session?",
    taskDefinition: "Run a multi-step diagnostic session where the same log file is relevant at two different points. Check whether the agent fetches it twice or uses prior context.",
    requiredData: "multi-step diagnostic session transcript, tool call logs with timestamps",
    candidateMeasure: "duplicate_fetch_rate: should be 0 for data already in context window",
    directionality: "lower_is_better" as const,
    riskLevel: "medium" as const,
    selected: false,
  },
  {
    ...
    question: "Does the agent complete a diagnostic session within the latency budget for CI-blocking use?",
    taskDefinition: "Run the agent on the standard diagnostic scenario set and measure wall-clock time from first tool call to structured report output.",
    requiredData: "10 standard scenarios, baseline latency from 10 prior runs",
    candidateMeasure: "session_duration_ms: P90 ≤ 45,000ms for CI-blocking cases",
    directionality: "lower_is_better" as const,
    riskLevel: "medium" as const,
    selected: false,
  },
  {
    id: "q11",
    rank: 11,
    showcaseCategory: "Stability" as const,
    question: "Does the agent produce consistent diagnoses when presented with semantically equivalent but syntactically different inputs?",
    taskDefinition: "Present the same underlying failure scenario in 5 different log formats (JSON, plaintext, CSV, verbose, terse). Compare root cause outputs.",
    requiredData: "same failure scenario expressed in 5 log format variants",
    candidateMeasure: "diagnosis_consistency: fraction of variants yielding the same root cause label",
    directionality: "higher_is_better" as const,
    riskLevel: "medium" as const,
    selected: false,
  },
  {
    ...
    rank: 12,
    showcaseCategory: "Stability" as const,
    question: "Does the agent produce a different diagnosis when irrelevant context is added to the prompt?",
    taskDefinition: "Add a paragraph of unrelated system context to a known-good diagnostic prompt. Compare diagnosis to the baseline.",
    requiredData: "baseline prompt with known correct diagnosis, same prompt + irrelevant context paragraph",
    candidateMeasure: "robustness_to_noise: diagnosis unchanged in ≥9 of 10 noise-injected runs",
    directionality: "higher_is_better" as const,
