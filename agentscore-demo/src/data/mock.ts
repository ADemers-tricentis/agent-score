...
import type {
  Project, Run, Session, Verdict, ProjectType, GuardLogEntry, EvalDesign, LLMJudge, ScoringProfile,
  ProfileVersion, Tenant, StaffUser, LLMPricingRow, LLMUsageLogEntry, LLMRoutingAssignment,
  RegistrySlot, RegistrySlotSlug, AgentVersion, SlotBinding, SlotBindingImpact,
  AccessRequest, AdviceRequest, AdvicePriority, Recommendation, AgentCard,
} from "../types";

// ── Mock data store ───────────────────────────────────────────────────────
// A minimal change-notification layer so mutating a mock array (approve a
// request, revoke a version, ...) re-renders every view reading it, without
// pulling in a real state-management dependency.

let mockVersion = 0;
const mockListeners = new Set<() => void>();

export function bumpMock() {
  mockVersion++;
  mockListeners.forEach((l) => l());
}

export function useMockData(): number {
...
export const ACCESS_REQUESTS: AccessRequest[] = [
  { id: "req-1", email: "m.reyes@tricentis.com", requestedAt: "2026-09-16T08:12:00Z", state: "waiting", declinedAt: null, approvedAt: null, existingUser: null },
  { id: "req-2", email: "j.kwon@tricentis.com", requestedAt: "2026-09-15T21:05:00Z", state: "waiting", declinedAt: "2026-09-10T09:00:00Z", approvedAt: null, existingUser: null },
  { id: "req-3", email: "l.cohen@tricentis.com", requestedAt: "2026-09-16T06:30:00Z", state: "waiting", declinedAt: null, approvedAt: null, existingUser: { userId: "u3", deleted: false } },
  { id: "req-4", email: "r.iyer@tricentis.com", requestedAt: "2026-09-14T13:47:00Z", state: "waiting", declinedAt: null, approvedAt: null, existingUser: { userId: "u-deleted-1", deleted: true } },
  { id: "req-5", email: "t.oyelaran@tricentis.com", requestedAt: "2026-09-12T10:15:00Z", state: "declined", declinedAt: "2026-09-12T15:00:00Z", approvedAt: null, existingUser: null },
  { id: "req-6", email: "c.nguyen@tricentis.com", requestedAt: "2026-09-08T11:20:00Z", state: "declined", declinedAt: "2026-09-08T17:30:00Z", approvedAt: null, existingUser: null },
  { id: "req-7", email: "s.oconnor@tricentis.com", requestedAt: "2026-09-05T09:00:00Z", state: "declined", declinedAt: "2026-09-05T14:10:00Z", approvedAt: null, existingUser: null },
  { id: "req-8", email: "e.marchetti@tricentis.com", requestedAt: "2026-08-20T09:00:00Z", state: "approved", declinedAt: null, approvedAt: "2026-08-20T12:00:00Z", existingUser: { userId: "u7", deleted: false } },
  { id: "req-9", email: "d.voss@wolterskluwer.com", requestedAt: "2026-07-15T09:00:00Z", state: "approved", declinedAt: null, approvedAt: "2026-07-15T13:00:00Z", existingUser: { userId: "u6", deleted: false } },
  { id: "req-10", email: "p.natarajan@meta.com", requestedAt: "2026-06-30T09:00:00Z", state: "approved", declinedAt: null, approvedAt: "2026-06-30T11:00:00Z", existingUser: { userId: "u5", deleted: false } },
];

let nextUserSeq = STAFF_USERS.length + 1;
...
export function removeAccessRequest(id: string) {
  const index = ACCESS_REQUESTS.findIndex((r) => r.id === id);
  if (index !== -1) ACCESS_REQUESTS.splice(index, 1);
  bumpMock();
}

export function createStaffUser(name: string, email: string, role: "admin" | "member", tenantIds: string[]): StaffUser {
  const user: StaffUser = { id: `u-new-${nextUserSeq++}`, name, email, role, tenantIds, lastActive: new Date().toISOString() };
  STAFF_USERS.push(user);
  bumpMock();
  return user;
}

export function approveAccessRequest(id: string, name: string, role: "admin" | "member", tenantIds: string[]): StaffUser {
  const req = ACCESS_REQUESTS.find((r) => r.id === id);
  const user: StaffUser = {
    id: `u-new-${nextUserSeq++}`,
    name,
    email: req?.email ?? "",
    role,
    tenantIds,
  ...
  if (req) {
    ...
    req.existingUser = { userId: user.id, deleted: false };
  }
  bumpMock();
  return user;
}
...
export function labelSession(projectId: string, sessionId: string) {
  const project = getProject(projectId);
  for (const run of project?.runs ?? []) {
    const session = run.sessions.find((s) => s.id === sessionId);
    if (session) {
      session.labeled = true;
      break;
    }
  }
  bumpMock();
...
const ADVICE_HISTORY = new Map<string, AdviceRequest[]>();
let nextAdviceSeq = 1;

const RECOMMENDATION_TEMPLATES: Record<string, { title: string; rationale: string; recommendation: string }[]> = {
  ATA: [
    { title: "Tighten baseline comparison tolerance", rationale: "Several sessions passed only because the baseline diff tolerance is wide enough to absorb a real regression.", recommendation: "Reduce the allowed baseline delta for the payment-flow assertions and re-run against the last 10 failing sessions." },
    { title: "Add a retry-storm guard", rationale: "The tool-use trace shows the agent retrying the same failing assertion 3+ times before giving up, inflating latency without changing the outcome.", recommendation: "Cap retries at 2 and fall back to a partial verdict instead of a silent pass." },
    { title: "Surface skipped assertions in the verdict", rationale: "Sessions marked PASS sometimes skip an assertion entirely rather than evaluating it, which looks identical to a real pass in the summary.", recommendation: "Have the agent explicitly report skipped assertions so PASS always means \"evaluated and correct.\"" },
  ],
  ATC: [
    { title: "Fix flaky coverage-gap detection", rationale: "The same source file is flagged as a coverage gap in one run and not the next, with no code change in between.", recommendation: "Cache the coverage baseline per commit instead of recomputing it per run to remove the nondeterminism." },
    { title: "Reduce redundant generated test cases", rationale: "Roughly a third of generated test cases duplicate an existing assertion almost verbatim.", recommendation: "De-duplicate generated cases against the existing suite before proposing them." },
  ],
  CURA: [
    { title: "Correlate alerts before escalating", rationale: "The agent emits a diagnosis report per alert even when three alerts in the same window share one root cause.", recommendation: "Batch alerts within a 5-minute window and correlate before generating a report." },
  ],
  AI_WORKSPACE: [
    { title: "Cite the source document more consistently", rationale: "Several PASS sessions produced a correct answer without a citation, which is indistinguishable from a hallucinated-but-correct answer.", recommendation: "Require an inline citation for any factual claim drawn from retrieved context." },
  ],
  CODING: [
    { title: "Reduce false-positive style violations", rationale: "About 15% of flagged style violations are on lines the diff didn't actually touch.", recommendation: "Scope the style check to changed lines only, not the whole file." },
  ],
  APT: [
    { title: "Widen the baseline comparison window", rationale: "P99 regressions are being flagged against a single prior run, which is noisy at this traffic volume.", recommendation: "Compare against a rolling 7-run median instead of the single most recent run." },
  ],
};

function buildRecommendations(project: Project): Recommendation[] {
  const templates = RECOMMENDATION_TEMPLATES[project.type] ?? RECOMMENDATION_TEMPLATES.ATA;
  const priorities: AdvicePriority[] = ["high", "medium", "low"];
  return templates.map((t, i) => ({
    ...t,
    priority: priorities[i % priorities.length],
    evidenceCount: 2 + di(project.id + t.title, 6, i + 1),
  }));
}

export function getAdviceHistory(projectId: string): AdviceRequest[] {
  return ADVICE_HISTORY.get(projectId) ?? [];
}

export function requestAdvice(projectId: string): AdviceRequest {
  const project = getProject(projectId)!;
  const request: AdviceRequest = {
    id: `advice-${nextAdviceSeq++}`,
    projectId,
    createdAt: new Date().toISOString(),
    status: "succeeded",
    recommendations: buildRecommendations(project),
  };
  const history = ADVICE_HISTORY.get(projectId) ?? [];
  history.unshift(request);
  ADVICE_HISTORY.set(projectId, history);
  bumpMock();
  return request;
}
...
// Seed one advice request for every project that already has a scored run, so the
// Improve tab has something to show on first load.
for (const project of PROJECTS) {
  if (project.runs.some((r) => r.status === "scored")) {
...
const AGENT_CARDS = new Map<string, AgentCard>();

const AGENT_CARD_PURPOSE: Record<string, string> = {
  ATA: "Runs the regression suite against each new build, replays recorded transactions, and reports pass/fail against expected outcomes.",
  ATC: "Analyzes test coverage gaps and drafts new test cases targeting the least-covered code paths.",
  CURA: "Watches production metrics and alerts, and synthesizes a root-cause hypothesis when something looks wrong.",
  AI_WORKSPACE: "Answers questions by retrieving relevant internal documents and drafting a grounded response.",
  CODING: "Reviews pull requests for style and correctness issues and posts inline review comments.",
  APT: "Tracks performance traces against a baseline and flags regressions before they reach production.",
};

function buildAgentCard(project: Project): AgentCard {
  const sessions = project.runs.flatMap((r) => r.sessions);
  const toolNames = ["run_test_suite", "get_test_results", "query_metrics", "fetch_diff"];
  const tools = toolNames.slice(0, 2 + di(project.id, 3, 1)).map((name, i) => ({
    name,
    calls: 12 + di(project.id + name, 40, i + 2),
    successRate: 0.82 + di(project.id + name, 15, i + 3) / 100,
  }));
  return {
    projectId: project.id,
    createdAt: new Date().toISOString(),
    purpose: AGENT_CARD_PURPOSE[project.type] ?? AGENT_CARD_PURPOSE.ATA,
    tools,
    behavioralPatterns: [
      "Runs a short planning step before invoking any tools.",
      "Retries a failing tool call once before surfacing an error.",
      sessions.some((s) => s.verdict === "FAIL") ? "Occasionally stops early when a critical assertion fails partway through." : "Completes the full evaluation sequence even on a partial pass.",
    ],
    successCriteria: [
      "All mandatory assertions evaluated, not skipped.",
      "No credentials or PII present in tool inputs or outputs.",
    ],
    failureModes: [
      "Can retry a flaky tool call enough times to inflate session latency.",
      "May report a pass when an assertion was skipped rather than evaluated.",
    ],
  };
}

export function getAgentCard(projectId: string): AgentCard | undefined {
  return AGENT_CARDS.get(projectId);
}

export function regenerateAgentCard(projectId: string): AgentCard {
  const project = getProject(projectId)!;
  const card = buildAgentCard(project);
  AGENT_CARDS.set(projectId, card);
  bumpMock();
  return card;
}
...
for (const project of PROJECTS) {
  if (project.runs.some((r) => r.status === "scored")) {
    AGENT_CARDS.set(project.id, buildAgentCard(project));
  }
}
...
export const REGISTRY_SLOTS: RegistrySlot[] = [
  {
    id: "slot-agent-card",
    slug: "agent_card",
    ...
    outputContract: { type: "object", properties: { purpose: { type: "string" }, tool_descriptions: { type: "object" } }, required: ["purpose"] },
    toolAllowlist: ["read_trace", "read_scoring_run"],
    readOnly: true,
  ...
  },
  {
    ...
    outputContract: { type: "object", properties: { score: { type: "number" }, reason: { type: "string" } }, required: ["score", "reason"] },
    toolAllowlist: ["read_trace", "read_profile"],
    readOnly: true,
  ...
  },
  {
    ...
    outputContract: { type: "object", properties: { recommendations: { type: "array" } }, required: ["recommendations"] },
    toolAllowlist: ["read_trace", "read_scoring_run", "read_advice_history"],
    readOnly: true,
    priorityClass: "background",
  },
  {
    ...
    outputContract: { type: "object", properties: { profileVersionId: { type: "string" }, confidence: { type: "number" } }, required: ["profileVersionId"] },
    toolAllowlist: ["read_trace", "list_profiles"],
    readOnly: true,
    priorityClass: "background",
  },
  {
    ...
    outputContract: { type: "object", properties: { message: { type: "string" } }, required: ["message"] },
    toolAllowlist: ["read_trace", "read_scoring_run", "read_profile", "read_agent_card"],
    readOnly: true,
    priorityClass: "interactive",
  },
];
...
export const AGENT_VERSIONS: AgentVersion[] = [
  // scoring_judge — a 3-deep global lineage, v3 currently bound.
  {
    id: "ver-judge-1", slotId: "slot-scoring-judge", tenantId: null, versionNumber: 1,
    name: "Initial judge", instructions: "Score the session against the active profile's dimensions and thresholds. Cite the spans that drove the verdict.",
    toolAllowlist: ["read_trace", "read_profile"], modelProvider: "anthropic", modelId: "claude-haiku-4-5-20251001",
    dollarCeilingUsd: "1.00", turnCap: 4, wallClockCapSeconds: 60,
    baseVersionId: null, createdAt: "2026-04-20T09:00:00Z", createdByUserId: null, revokedAt: null,
  },
  {
    id: "ver-judge-2", slotId: "slot-scoring-judge", tenantId: null, versionNumber: 2,
    name: "Sonnet judge", instructions: "Score the session against the active profile's dimensions and thresholds. Cite the spans that drove the verdict. Prefer conservative verdicts on ambiguous evidence.",
    toolAllowlist: ["read_trace", "read_profile"], modelProvider: "anthropic", modelId: "claude-sonnet-4-6",
    dollarCeilingUsd: "3.00", turnCap: 4, wallClockCapSeconds: 60,
    baseVersionId: "ver-judge-1", createdAt: "2026-06-10T14:30:00Z", createdByUserId: "u-andrew", revokedAt: null,
  },
  {
    id: "ver-judge-3", slotId: "slot-scoring-judge", tenantId: null, versionNumber: 3,
    name: "Sonnet judge — evidence caps", instructions: "Score the session against the active profile's dimensions and thresholds. Cite the spans that drove the verdict. Prefer conservative verdicts on ambiguous evidence. Cap cited evidence at 5 spans per dimension.",
    toolAllowlist: ["read_trace", "read_profile"], modelProvider: "anthropic", modelId: "claude-sonnet-4-6",
    dollarCeilingUsd: "3.00", turnCap: 5, wallClockCapSeconds: 90,
    baseVersionId: "ver-judge-2", createdAt: "2026-08-22T11:15:00Z", createdByUserId: "u-andrew", revokedAt: null,
  },
  // improvement_advisor — a single version, revoked, but still the one bound (drives the
  // "cannot serve traffic" state on the slot).
  {
    id: "ver-advisor-1", slotId: "slot-improvement-advisor", tenantId: null, versionNumber: 1,
    name: "Opus advisor", instructions: "Read the run's verdict and failure summary, then produce up to 4 prioritized recommendations, each grounded in specific trace evidence.",
    toolAllowlist: ["read_trace", "read_scoring_run", "read_advice_history"], modelProvider: "anthropic", modelId: "claude-opus-4-8",
    dollarCeilingUsd: "5.00", turnCap: 6, wallClockCapSeconds: 120,
    baseVersionId: null, createdAt: "2026-05-02T10:00:00Z", createdByUserId: "u-andrew", revokedAt: "2026-09-10T16:00:00Z",
  },
  // agent_card — a version exists but nothing is bound to it (drives the "Unbound" state).
  {
    id: "ver-card-1", slotId: "slot-agent-card", tenantId: null, versionNumber: 1,
    name: "Card writer draft", instructions: "Summarize the agent's observed tool usage and infer its purpose, typical behavior, success criteria, and failure modes from the sampled traces.",
    toolAllowlist: ["read_trace", "read_scoring_run"], modelProvider: "anthropic", modelId: "claude-sonnet-4-6",
    dollarCeilingUsd: "2.00", turnCap: 3, wallClockCapSeconds: 60,
    baseVersionId: null, createdAt: "2026-05-15T08:00:00Z", createdByUserId: null, revokedAt: null,
  },
  // profile_fit — global v1, then a newer global v2; tenant "tais" still overrides to v1
  // (source: tenant, and stale since the global binding has since moved to v2).
  {
    id: "ver-fit-1", slotId: "slot-profile-fit", tenantId: null, versionNumber: 1,
    name: "Fit matcher", instructions: "Compare the agent's observed tool usage and turn structure against each candidate profile and choose the closest match.",
    toolAllowlist: ["read_trace", "list_profiles"], modelProvider: "anthropic", modelId: "claude-haiku-4-5-20251001",
    dollarCeilingUsd: "1.50", turnCap: 3, wallClockCapSeconds: 45,
    baseVersionId: null, createdAt: "2026-05-20T09:00:00Z", createdByUserId: null, revokedAt: null,
  },
  {
    id: "ver-fit-2", slotId: "slot-profile-fit", tenantId: null, versionNumber: 2,
    name: "Fit matcher — confidence scoring", instructions: "Compare the agent's observed tool usage and turn structure against each candidate profile, choose the closest match, and report a confidence score with rationale.",
    toolAllowlist: ["read_trace", "list_profiles"], modelProvider: "anthropic", modelId: "claude-haiku-4-5-20251001",
    dollarCeilingUsd: "1.50", turnCap: 3, wallClockCapSeconds: 45,
    baseVersionId: "ver-fit-1", createdAt: "2026-08-05T13:00:00Z", createdByUserId: "u-andrew", revokedAt: null,
  },
  // chat_assistant — a single plain global version, bound.
  {
    id: "ver-chat-1", slotId: "slot-chat-assistant", tenantId: null, versionNumber: 1,
    name: "Chat assistant", instructions: "Answer questions about this tenant's agents, scores, and runs in plain language. Cite the run or trace you're referencing.",
    toolAllowlist: ["read_trace", "read_scoring_run", "read_profile", "read_agent_card"], modelProvider: "anthropic", modelId: "claude-sonnet-4-6",
    dollarCeilingUsd: "3.00", turnCap: 8, wallClockCapSeconds: 120,
    baseVersionId: null, createdAt: "2026-04-25T10:00:00Z", createdByUserId: null, revokedAt: null,
  },
];

export const SLOT_BINDINGS: SlotBinding[] = [
  { id: "bind-judge-global", slotId: "slot-scoring-judge", tenantId: null, agentVersionId: "ver-judge-3", createdAt: "2026-08-22T11:15:00Z", updatedAt: "2026-08-22T11:15:00Z", updatedByUserId: "u-andrew" },
  { id: "bind-advisor-global", slotId: "slot-improvement-advisor", tenantId: null, agentVersionId: "ver-advisor-1", createdAt: "2026-05-02T10:00:00Z", updatedAt: "2026-05-02T10:00:00Z", updatedByUserId: "u-andrew" },
  { id: "bind-fit-global", slotId: "slot-profile-fit", tenantId: null, agentVersionId: "ver-fit-2", createdAt: "2026-08-05T13:00:00Z", updatedAt: "2026-08-05T13:00:00Z", updatedByUserId: "u-andrew" },
  { id: "bind-fit-tais", slotId: "slot-profile-fit", tenantId: "tais", agentVersionId: "ver-fit-1", createdAt: "2026-05-21T09:00:00Z", updatedAt: "2026-05-21T09:00:00Z", updatedByUserId: "u-andrew" },
  { id: "bind-chat-global", slotId: "slot-chat-assistant", tenantId: null, agentVersionId: "ver-chat-1", createdAt: "2026-04-25T10:00:00Z", updatedAt: "2026-04-25T10:00:00Z", updatedByUserId: null },
  // agent_card has no binding at any scope — the slot is Unbound everywhere.
];

let nextVersionSeq = AGENT_VERSIONS.length + 1;
let nextBindingSeq = SLOT_BINDINGS.length + 1;

export function registrySlots(): RegistrySlot[] {
  return REGISTRY_SLOTS;
}

export function getRegistrySlot(slotSlug: string): RegistrySlot | undefined {
  return REGISTRY_SLOTS.find((s) => s.slug === slotSlug);
}

function bindingFor(slotId: string, tenantId: string | null): SlotBinding | undefined {
  return SLOT_BINDINGS.find((b) => b.slotId === slotId && b.tenantId === tenantId);
}

export function versionsForSlot(slotId: string, tenantId: string | null): AgentVersion[] {
  return AGENT_VERSIONS
    .filter((v) => v.slotId === slotId && (v.tenantId === tenantId || (tenantId !== null && v.tenantId === null)))
...
export interface ResolvedSlotBinding {
  ...
  source: "tenant" | "global" | null;
  stale: boolean;
}

/** Resolves what a slot actually serves in a scope: the tenant's own binding if it has
 * one, else the global binding as fallback. `stale` mirrors production - true when a
 * tenant override's global ancestor is no longer what the global binding currently serves. */
export function resolveSlotBinding(slotId: string, tenantId: string | null): ResolvedSlotBinding {
  if (tenantId === null) {
    const binding = bindingFor(slotId, null);
    const version = binding ? AGENT_VERSIONS.find((v) => v.id === binding.agentVersionId) : undefined;
    return { binding, version, source: binding ? "global" : null, stale: false };
  }

  const tenantBinding = bindingFor(slotId, tenantId);
  if (tenantBinding) {
    const version = AGENT_VERSIONS.find((v) => v.id === tenantBinding.agentVersionId);
    const globalBinding = bindingFor(slotId, null);
    const stale = Boolean(
      version?.tenantId === null && globalBinding && globalBinding.agentVersionId !== version.id,
    );
    return { binding: tenantBinding, version, source: "tenant", stale };
  }

  const globalBinding = bindingFor(slotId, null);
  const version = globalBinding ? AGENT_VERSIONS.find((v) => v.id === globalBinding.agentVersionId) : undefined;
  return { binding: globalBinding, version, source: globalBinding ? "global" : null, stale: false };
}

export function bindableVersions(slotId: string, tenantId: string | null): AgentVersion[] {
  const tenantVersions = tenantId !== null ? AGENT_VERSIONS.filter((v) => v.slotId === slotId && v.tenantId === tenantId) : [];
  const globalVersions = AGENT_VERSIONS.filter((v) => v.slotId === slotId && v.tenantId === null);
  return [...tenantVersions, ...globalVersions].sort((a, b) => b.versionNumber - a.versionNumber);
}

export function createAgentVersion(input: {
  slotSlug: string; tenantId: string | null; fromVersionId?: string;
  name: string; instructions: string; toolAllowlist: string[];
  modelProvider: string; modelId: string; dollarCeilingUsd: string;
  turnCap: number | null; wallClockCapSeconds: number | null;
}): AgentVersion {
  const slot = getRegistrySlot(input.slotSlug)!;
  const existing = AGENT_VERSIONS.filter((v) => v.slotId === slot.id && v.tenantId === input.tenantId);
  const versionNumber = existing.length > 0 ? Math.max(...existing.map((v) => v.versionNumber)) + 1 : 1;
  const version: AgentVersion = {
    id: `ver-new-${nextVersionSeq++}`,
    slotId: slot.id,
    ...
    dollarCeilingUsd: input.dollarCeilingUsd,
    turnCap: input.turnCap,
    wallClockCapSeconds: input.wallClockCapSeconds,
    baseVersionId: input.fromVersionId ?? null,
    createdAt: new Date().toISOString(),
    createdByUserId: "u-andrew",
    revokedAt: null,
  };
...
export function reassignBinding(slotId: string, tenantId: string | null, agentVersionId: string) {
  const existing = bindingFor(slotId, tenantId);
  if (existing) {
    existing.agentVersionId = agentVersionId;
    existing.updatedAt = new Date().toISOString();
    existing.updatedByUserId = "u-andrew";
  } else {
    SLOT_BINDINGS.push({
      id: `bind-new-${nextBindingSeq++}`,
      slotId,
      tenantId,
      agentVersionId,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      updatedByUserId: "u-andrew",
    });
  }
  bumpMock();
}

export function bindingsForVersion(versionId: string): SlotBindingImpact[] {
  return SLOT_BINDINGS.filter((b) => b.agentVersionId === versionId)
    .map((b) => {
      const slot = REGISTRY_SLOTS.find((s) => s.id === b.slotId)!;
      return { id: `slot-local:${b.id}`, slotSlug: slot.slug, tenantId: b.tenantId, isGlobal: b.tenantId === null } as SlotBindingImpact;
    })
    .sort((a, b) => Number(b.isGlobal) - Number(a.isGlobal));
}

export function revokeAgentVersion(versionId: string): { affectedBindingCount: number; affectedBindings: SlotBindingImpact[] } {
  const version = AGENT_VERSIONS.find((v) => v.id === versionId);
  if (version) version.revokedAt = new Date().toISOString();

  const affected = SLOT_BINDINGS.filter((b) => b.agentVersionId === versionId).map((b) => {
    const slot = REGISTRY_SLOTS.find((s) => s.id === b.slotId)!;
    return { id: `slot-local:${b.id}`, slotSlug: slot.slug, tenantId: b.tenantId, isGlobal: b.tenantId === null } as SlotBindingImpact;
  });

  bumpMock();
  return { affectedBindingCount: affected.length, affectedBindings: affected };
}

/** A value `> 0 && < 0.01` prints verbatim rather than rounding to `$0.00`, matching
 * production's `format.ts`. */
