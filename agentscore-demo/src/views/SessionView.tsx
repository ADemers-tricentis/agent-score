  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1100 }}>
      ...
      <Box sx={{ display: "flex", gap: 0.5, alignItems: "center", mb: 2 }}>
        ...
        <Button size="small" onClick={() => navigate({ name: "agent", projectId })} sx={{ color: "text.disabled" }}>
          {project.name}
        </Button>
        <Typography sx={{ color: "text.disabled" }}>/</Typography>
        <Button size="small" onClick={() => navigate({ name: "agent-run", projectId, runId })} sx={{ color: "text.secondary" }}>
          {run.label}
        </Button>
      </Box>
