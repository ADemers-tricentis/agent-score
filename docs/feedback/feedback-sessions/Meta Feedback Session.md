...
Sri asked, unprompted, how to test a *group* of agents against a defined scenario/context, not just one agent in isolation - a direct hit on his functional -> value-stream -> enterprise hierarchy. David's answer (measure the collective for business-level trajectory, measure each agent individually to pinpoint which one caused the failure) drew an explicit "that makes sense, okay, cool" - the clearest unprompted buy-signal in the call. The question actually had two halves - Sri also asked how you'd *drive* a defined scenario into the agent in the first place (provide the context, set expectations, then test against it) - and David offered a partial answer to that half too, pointing to Tosca as the mechanism for stimulating/simulating the scenario inputs themselves. That half-answer never got surfaced as its own point; it's a real, if informal, response to the same scenario-definition question raised later at Meta POC-scoping and by L'Oreal, not a flat "not built."
