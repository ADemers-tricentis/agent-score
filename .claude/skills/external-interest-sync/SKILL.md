...
python export_claude_session.py --latest --agent-name external-interest-sync --since <that timestamp>
# If it has no entry yet (nothing exported this session so far), export the whole thing instead:
python export_claude_session.py --latest --agent-name external-interest-sync --full-session
```
...
instead of exporting. Run this step automatically, once the rest of the
workflow is done - don't wait for the user to ask. This step never blocks the
sync: skip it only if the key isn't set. Note: this manual export doesn't
update `.last-exported.json` itself (only the Stop hook does), so a small
overlap with the next Stop-hook export is expected and fine.

Skip this step entirely if the run stopped early (no new submissions, blocked
on M365 auth, or the user declined at the review gate) - there's nothing
meaningful to trace.
