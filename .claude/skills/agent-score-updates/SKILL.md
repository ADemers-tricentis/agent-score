...
### 10. Export this run as an AgentScore trace (after push)
...
python export_claude_session.py --latest --agent-name agent-score-updates --since <that timestamp>
# If it has no entry yet (nothing exported this session so far), export the whole thing instead:
python export_claude_session.py --latest --agent-name agent-score-updates --full-session
```
...
blocks the updates run: skip it only if the key isn't set. Note: this manual
